...
print(f"   ► Base Fare: ₹{config.base_fare} (includes first {config.base_distance_km}km)")
print(f"   ► Per KM Rate: ₹{config.per_km_rate}/km")
print(f"   ► Waiting Fee: ₹{config.waiting_per_minute}/min (after {config.waiting_free_minutes}m free)\n")

# 2. Setup mock users
rider_user, _ = User.objects.get_or_create(phone="+10000000001", defaults={"username": "test_rider_1", "first_name": "Test", "last_name": "Rider", "role": "RIDER"})
driver_user, _ = User.objects.get_or_create(phone="+10000000002", defaults={"username": "test_driver_1", "first_name": "Test", "last_name": "Driver", "role": "DRIVER"})
driver, _ = Driver.objects.get_or_create(user=driver_user, defaults={"status": "ONLINE", "is_verified": True})
...
print(f"📍 Need a ride from Bangalore Central to Koramangala (5km)")
print("==================================\n")

# 4. Driver Arrives (Start Waiting Time)
update_ride_status(ride, Ride.Status.ARRIVED)
ride.arrived_at = timezone.now() - timedelta(minutes=7) # Pretend driver arrived 7 mins ago
ride.save(update_fields=["arrived_at"])
print(f"🚘 Driver Arrived 7 minutes ago.")
...
print(f"⏱️ Redis tracked vehicle wait: {simulated_waiting_seconds/60} mins outside (420 seconds)")
...
print(f"🟢 Rider boards. Trip Starts!\n")
...
print(f"🏁 Driver completes trip. Actual Distance: 8.5km\n")
...
print(f"   Extra {Decimal('8.5') - config.base_distance_km}km @ ₹{config.per_km_rate}/km: ₹{breakdown.get('distance_charge')}")

print(f"\n👉 Waiting Charge (7 minutes):")
print(f"   First {config.waiting_free_minutes} minutes: Free")
print(f"   Extra 5 minutes @ ₹{config.waiting_per_minute}/min: ₹{breakdown.get('waiting_charge')}")
