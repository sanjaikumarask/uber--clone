...
def evaluate_level(driver: Driver):
    ...
    if stats.is_level_overridden:
        logger.info(
            f"Driver {driver.id} has active admin override until {stats.level_override_until}"
        )
        return
    ...
    if (
        stats.weekly_rides >= 100
        and stats.acceptance_rate >= 90
        and stats.cancellation_rate < 10
    ):
        new_level = Driver.Level.PRO
        reason = "AUTO_PROMOTION"

    elif stats.weekly_rides >= 50 and stats.acceptance_rate >= 80:
    ...
    if 60 <= stats.acceptance_rate < 75:
        Notification.objects.create(
            ...
            payload={
                ...
                "message": "Your acceptance rate is dropping. Accept more rides to maintain your level.",
            },
        )
    ...
    if 15 < stats.cancellation_rate <= 25:
        Notification.objects.create(
            ...
            payload={
                ...
                "message": "Your cancellation rate is high. This may lead to a level downgrade.",
            },
        )
