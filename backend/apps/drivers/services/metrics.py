...
def update_driver_metrics(driver: Driver, event_type: str, **kwargs):
    """
    ...
    if event_type == "OFFERED":
        ...
        update_fields.extend(["completed_rides", "total_rides", "weekly_rides", "peak_hour_rides"])
    ...
    if stats.offered_rides > 0:
        stats.acceptance_rate = round((stats.accepted_rides / stats.offered_rides) * 100, 2)
        update_fields.append("acceptance_rate")
    
    if stats.accepted_rides > 0:
        stats.cancellation_rate = round((stats.cancelled_rides / stats.accepted_rides) * 100, 2)
