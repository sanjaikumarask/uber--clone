...
class AdminDriversListView(APIView):
    ...
    def get(self, request):
        qs = Driver.objects.select_related("user", "stats").all().order_by("-stats__score")
        ...
        for d in qs:
            ...
            data.append({
                ...
                "suspended_until":  stats.suspended_until.isoformat() if stats and stats.suspended_until else None,
...
class AdminDriverDetailView(APIView):
    ...
    def get(self, request, driver_id):
        d = get_object_or_404(Driver.objects.select_related("user", "stats"), id=driver_id)
        stats = getattr(d, "stats", None)

        level_history = DriverLevelHistory.objects.filter(driver=d).select_related("changed_by").order_by("-created_at")[:20]
...
class AdminDriverActionView(APIView):
    ...
    def post(self, request):
        ...
        VALID_ACTIONS = {"suspend", "unsuspend", "block", "unblock", "force_offline", "recalculate_score"}
        if not driver_id or action not in VALID_ACTIONS:
            return Response({"error": f"Invalid params. action must be one of: {VALID_ACTIONS}"}, status=400)
        ...
        logger.info(f"Admin {request.user.id} performed action '{action}' on driver {driver_id}")
        return Response({"success": True, "status": driver.status, "is_suspended": stats.is_suspended})
...
class AdminDriverLevelView(APIView):
    ...
    def post(self, request, driver_id):
        ...
        if new_level not in Driver.Level.values:
            return Response({"error": f"Invalid level. Choose from: {Driver.Level.values}"}, status=400)

        try:
            duration_days = int(request.data.get("duration_days", 7))
            admin_set_level(driver, new_level, admin_user=request.user, reason=reason, duration_days=duration_days)
...
class AdminDriverLevelHistoryView(APIView):
    ...
    def get(self, request, driver_id):
        ...
        history = DriverLevelHistory.objects.filter(driver_id=driver_id).select_related("changed_by").order_by("-created_at")[:50]
        data = [
            {
                "old_level":  h.old_level,
                "new_level":  h.new_level,
...
class AdminDriverRidesHistoryView(APIView):
    ...
    def get(self, request, driver_id):
        ...
        rides = Ride.objects.filter(driver_id=driver_id).select_related("rider", "driver__user").order_by("-created_at")[:50]
...
class AdminPendingDocumentsView(APIView):
    ...
    def get(self, request):
        ...
        docs = DriverDocument.objects.filter(status=DriverDocument.Status.PENDING).select_related("driver", "driver__user")
        data = []
        for doc in docs:
...
class AdminPendingDriversView(APIView):
    ...
    def get(self, request):
        from apps.drivers.models import DriverDocument
        drivers = Driver.objects.filter(is_verified=False).order_by("-created_at").prefetch_related("documents").select_related("user")
        data = []
        for d in drivers:
