...
class GoOnlineView(APIView):
    ...
    def post(self, request):
        ...
        if (
            ...
            and stats.suspended_until > timezone.now()
        ):
            return Response(
                {
...
class UpdateLocationView(APIView):
    ...
    def post(self, request):
        ...
        if active_ride:
            ...
            if (
                active_ride.status == Ride.Status.ONGOING
                and active_ride.planned_route_polyline
            ):
                from apps.rides.services.deviation import check_route_deviation

                is_deviated, _ = check_route_deviation(
                    driver, active_ride, lat, lng
                )
...
class DriverStatusView(APIView):
    ...
    def post(self, request):
        ...
        if new_status == "OFFLINE" and Ride.objects.filter(
            driver=driver,
            status__in=[
                Ride.Status.ASSIGNED,
                Ride.Status.ARRIVED,
...
class DocumentUploadView(APIView):
    ...
    def get(self, request):
        ...
        return Response(
            DriverDocumentSerializer(docs, many=True, context={"request": request}).data
        )
    ...
    def post(self, request):
        ...
        if not doc_type or not file:
            return Response(
                {"error": "document_type and file are required"}, status=400
            )
        ...
        update_data = {"status": DriverDocument.Status.PENDING, "rejection_reason": ""}
        ...
        try:
            ...
            print("[ERROR] Failed to create media directory")

        doc, created = DriverDocument.objects.update_or_create(
            driver=driver, document_type=doc_type, defaults=update_data
        )
...
class DriverRideHistoryView(APIView):
    ...
    def get(self, request):
        ...
        rides = (
            Ride.objects.filter(driver=request.user.driver)
            .exclude(status__in=[Ride.Status.SEARCHING, Ride.Status.OFFERED])
            .select_related("rider")
