...
from apps.supports.models import Emergency, SupportTicket
...
class AdminLedgerView(APIView):
    ...
    def get(self, request):
        ...
        for r in entries:
            ...
            phone = getattr(r.user, "phone", None) or getattr(
                r.user, "username", str(r.user.id)
            )
            data.append(
                {
                    "id": r.id,
...
class AdminPayoutListView(APIView):
    ...
    def get(self, request):
        ...
        data = []
        for p in payouts:
            phone = getattr(p.driver, "phone", None) or getattr(
                p.driver, "username", str(p.driver.id)
            )
            data.append(
                {
                    "id": p.id,
...
class AdminPayoutActionView(APIView):
    ...
    def post(self, request, action, payout_id):
        payout = get_object_or_404(Payout, id=payout_id)

        if action == "approve":
            if payout.status not in [Payout.Status.REQUESTED, Payout.Status.FAILED]:
                return Response(
                    {"error": "Can only approve REQUESTED or FAILED payouts"},
                    status=400,
                )
            ...
            if payout.status != Payout.Status.REQUESTED:
                return Response(
                    {"error": "Can only reject REQUESTED payouts"}, status=400
                )
...
class AdminTicketsView(APIView):
    ...
    def get(self, request):
        tickets = (
            SupportTicket.objects.filter(status=SupportTicket.Status.OPEN)
            .select_related("ride", "user")
            .order_by("-created_at")[:50]
        )
        emergencies = (
            Emergency.objects.filter(status=Emergency.Status.ACTIVE)
            .select_related("ride", "user")
        ...
        for t in tickets:
            phone = (
                getattr(t.user, "phone", None)
                or getattr(t.user, "username", str(t.user.id))
                if t.user
                else "Unknown"
            )
            t_data.append(
        ...
        for e in emergencies:
            phone = (
                getattr(e.user, "phone", None)
                or getattr(e.user, "username", str(e.user.id))
                if e.user
                else "Unknown"
            )
            e_data.append(
        ...
        return Response({"tickets": t_data, "emergencies": e_data})
