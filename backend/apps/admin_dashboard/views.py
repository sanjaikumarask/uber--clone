...
from apps.payments.models import Payment, LedgerEntry, DriverEarnings
...
class AdminLiveRidesView(APIView):
    ...
    def get(self, request):
        ...
        rides = Ride.objects.filter(status__in=active_statuses).select_related("rider", "driver", "driver__user")
...
class AdminPaymentStatusView(APIView):
    ...
    def get(self, request):
        ...
        total_revenue = payments.filter(status=Payment.Status.CAPTURED).aggregate(Sum('amount'))['amount__sum'] or 0
...
class AdminAnalyticsView(APIView):
    ...
    def get(self, request):
        ...
        rides_stats = Ride.objects.filter(
            ...
            avg_distance=Avg('actual_distance_km', filter=Q(status=Ride.Status.COMPLETED))
        ).order_by('date')

        # Compute lifetime stats
        total_revenue = Ride.objects.filter(status=Ride.Status.COMPLETED).aggregate(Sum('final_fare'))['final_fare__sum'] or 0
        ...
        overall_cancellation_rate = (total_cancelled / max(total_completed + total_cancelled, 1)) * 100
...
class AdminAlertsView(APIView):
    ...
    def get(self, request):
        ...
        for r in stuck_rides:
             ...
             if not SystemLog.objects.filter(type=SystemLog.LogType.RIDE_STUCK, metadata__ride_id=r.id).exists():
...
class AdminDriverActionView(APIView):
    ...
    def post(self, request, driver_id):
        ...
        if action == "approve":
            ...
            driver.save()
        elif action == "reject" or action == "block":
...
class AdminLiveMapSnapshot(APIView):
    ...
    def get(self, request):
        ...
        return Response({
            ...
            } for d in drivers if d.last_lat and d.last_lng],
            "rides": [{
                "id": r.id,
                "status": r.status,
...
class AdminOverviewView(APIView):
    ...
    def get(self, request):
        ...
        return Response({
            ...
            "active_rides": Ride.objects.filter(status__in=[Ride.Status.SEARCHING, Ride.Status.OFFERED, Ride.Status.ASSIGNED, Ride.Status.ARRIVED, Ride.Status.ONGOING]).count(),
