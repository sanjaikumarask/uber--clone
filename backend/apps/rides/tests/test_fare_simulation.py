...
class EndToEndFareTests(TestCase):
    ...
    def test_heavy_traffic_waiting_charge(self):
        ...
        cache.set(
            _cache_key(ride.id), {"accumulated_secs": 480, "is_waiting": False}, 60
        )

        update_ride_status(ride, Ride.Status.ONGOING)
        ride.otp_verified_at = timezone.now()
        ride.actual_distance_km = Decimal(
            "6.5"
        )  # (6.5km total - 2km base) = 4.5km billable
        ride.save()
        ...
        self.assertEqual(
            completed_ride.fare_breakdown["distance_charge"], "90.00"
        )  # 4.5km * 20
        self.assertEqual(
            completed_ride.fare_breakdown["waiting_charge"], "30.00"
        )  # (8m-2m free) * 5
