...
RIDE_REQUEST_URL = "/api/rides/request/"
...
class TestRideBookingIntegration:
    ...
    def test_successful_ride_booking_flow(
        self, mock_get, authenticated_rider_client, setup_driver
    ):
        """
        ...
        with patch("apps.drivers.services.geo.get_nearby_driver_ids") as mock_geo:
            ...
            # 1. API Call
            response = authenticated_rider_client.post(RIDE_REQUEST_URL, payload)
            ...
            assert ride.planned_distance_km == pytest.approx(5.0)
            ...
            with (
                ...
                patch(
                    "apps.drivers.services.geo.lock_driver_for_offer", return_value=True
                ),
            ):
    ...
    def test_ride_booking_idempotency(self, authenticated_rider_client):
        ...
        # First request
        resp1 = authenticated_rider_client.post(
            RIDE_REQUEST_URL, payload, **headers
        )
        assert resp1.status_code == status.HTTP_201_CREATED

        # Second identical request
        resp2 = authenticated_rider_client.post(
            RIDE_REQUEST_URL, payload, **headers
        )
    ...
    def test_no_driver_available_scenario(
        self, mock_matching, authenticated_rider_client
    ):
