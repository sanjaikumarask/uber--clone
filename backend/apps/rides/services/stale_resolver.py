...
def auto_resolve_stale_rides():
    ...
    stale_searching = Ride.objects.filter(
        status=Ride.Status.SEARCHING, created_at__lt=searching_cutoff
    )
    for ride in stale_searching:
        cancel_ride(ride, by=Ride.CancelledBy.SYSTEM)
        logger.warning(
            f"AUTO: Cancelled stale searching ride {ride.id}",
    ...
    for ride in abandoned_rides:
        try:
            if ride.status == Ride.Status.ONGOING:
                ...
                logger.warning(
                    f"AUTO: Completed abandoned ongoing ride {ride.id}",
                    extra={"ride_id": ride.id},
                )
            else:
                cancel_ride(ride, by=Ride.CancelledBy.SYSTEM)
                logger.warning(
                    f"AUTO: Cancelled abandoned arrived/assigned ride {ride.id}",
                    extra={"ride_id": ride.id},
                )
        except Exception as e:
            logger.error(
                f"AUTO: Failed to resolve abandoned ride {ride.id}: {e}",
                extra={"ride_id": ride.id},
            )
