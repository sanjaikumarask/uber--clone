...
from apps.rides.services.surge import get_surge
...
def calculate_final_fare(ride) -> Decimal:
    ...
    try:
        raw_surge = Decimal(str(get_surge(ride.pickup_lat, ride.pickup_lng)))
    except Exception:
    ...
    if ride.base_fare and actual_km <= Decimal(
        str(ride.planned_distance_km or 0)
    ) * Decimal("1.2"):
...
def get_fare_breakdown(ride) -> dict:
    ...
    try:
        surge = Decimal(str(get_surge(ride.pickup_lat, ride.pickup_lng)))
    except Exception:
