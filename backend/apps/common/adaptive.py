...
class AdaptiveShedder:
    ...
    def update_health(cls):
        ...
        if latency > cls.LATENCY_THRESHOLD or budget_exhausted:
            logger.warning(f"[Adaptive] Health Degraded (Latency: {latency:.4f}s, BudgetExhausted: {budget_exhausted})")
    ...
    def should_shed(cls, priority: str = "NORMAL") -> bool:
        ...
        if factor > limit:
            logger.error(f"[Adaptive] SHEDDING LOAD (Priority: {priority}, Factor: {factor:.2f})")
            return True
