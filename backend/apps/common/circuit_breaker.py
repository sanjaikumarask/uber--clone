    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if cache.get(open_key):
                raise CircuitOpenException(f"Circuit Breaker OPEN for {service_name}. Bypassing external call.")
            ...
            try:
                ...
                logger.warning(f"Circuit Breaker ({service_name}) failure {fails}/{failure_threshold}: {e}")
                
                if fails >= failure_threshold:
                    # Trip the breaker -> Transition to OPEN state
                    logger.critical(f"🛑 CIRCUIT BREAKER TRIPPED for {service_name}! Freezing calls for {recovery_timeout}s.")
