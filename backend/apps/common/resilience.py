...
class CircuitBreaker:
        ...
        def wrapper(*args, **kwargs):
            ...
            if state == "OPEN":
                if not cache.get(f"{self.state_key}:timer"):
                    ...
                    logger.warning(
                        f"[Resilience] Circuit {self.name} is OPEN. Rejecting request."
                    )
    ...
    def _handle_success(self, state):
        if state == "HALF_OPEN":
    ...
    def _handle_failure(self, state):
        if state == "HALF_OPEN":
            logger.error(f"[Resilience] Circuit {self.name} failed trial. Re-opening.")
            self._open()
        else:
            failures = (
                cache.incr(self.fail_key, 1)
                if cache.get(self.fail_key)
                else cache.set(self.fail_key, 1) or 1
            )
            if failures >= self.threshold:
                logger.critical(
                    f"[Resilience] Circuit {self.name} threshold reached ({failures}). Opening."
                )
                self._open()
