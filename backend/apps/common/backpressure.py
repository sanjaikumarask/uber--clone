...
class ConnectionRateLimiter:
    ...
    def is_allowed(cls, driver_id: int) -> bool:
        ...
        try:
            ...
            logger.exception("[Backpressure] Rate limiter Redis error")
...
def endpoint_cooldown(
    ...
    try:
        ...
        member = f"{now}:{_cryptogen.getrandbits(32)}"
        pipe = r.pipeline()
...
class CeleryQueueGuard:
    ...
    def can_enqueue(cls, queue_name: str = "medium") -> bool:
        ...
        try:
            ...
            if depth > limit:
                logger.warning(
                    f"[Backpressure] Queue {queue_name} saturated (depth={depth})"
                )
                return False
            return True
        except Exception:
...
class RetryStrategy:
    ...
    def get_countdown(cls, priority: str, attempt: int) -> float:
        ...
        jitter = _cryptogen.uniform(0, capped)
