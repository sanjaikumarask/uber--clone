    ...
    def __call__(self, request):
        ...
        try:
            ...
            if not cache.add(cache_key, "IN_FLIGHT", timeout=60):
                ...
                if cached == "IN_FLIGHT":
                    logger.warning(f"[Idempotency] IN_FLIGHT duplicate: user={user_id} key={idem_key}")
                    return JsonResponse(
                        {"error": "Request already in progress. Retry after a moment."},
                        status=status.HTTP_409_CONFLICT,
                    )
                logger.info(f"[Idempotency] Replaying cached response: user={user_id} key={idem_key}")
                return JsonResponse(cached["data"], status=cached["status"])

        except Exception as exc:
            logger.critical(f"[Idempotency] Redis failure (pre): {exc}")
        ...
        try:
            ...
            logger.critical(f"[Idempotency] Redis failure (post): {exc}")
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if not cache.add(running_key, "1", timeout=120):
                logger.warning(f"[Idempotency] Task already running: {func_name}")
                return None

            try:
                # Phase 2: reject already-completed tasks in TTL window
                if cache.get(done_key):
                    logger.info(f"[Idempotency] Task already done (replay blocked): {func_name}")
                    return None
    ...
    def decorator(view_func):
        ...
        def wrapper(request, *args, **kwargs):
            ...
            if cache.get(cache_key):
                logger.info(f"[Idempotency] Duplicate webhook dropped: {provider}/{event_id}")
                return JsonResponse({"status": "already_processed"}, status=200)

            # Concurrent duplicate?
            if not cache.add(lock_key, "1", timeout=60):
                logger.warning(f"[Idempotency] Concurrent webhook: {provider}/{event_id}")
...
def idempotent_request(ttl=3600):
    ...
    def decorator(view_func):
        ...
        def wrapper(self, request, *args, **kwargs):
            # 1. Non-mutating requests skip idempotency.
            if request.method not in ("POST", "PUT", "PATCH"):
                return view_func(self, request, *args, **kwargs)

            # 2. No key → normal execution.
            idem_key = request.headers.get("X-Idempotency-Key")
            if not idem_key:
                return view_func(self, request, *args, **kwargs)
            ...
            try:
                ...
                if not cache.add(cache_key, "IN_FLIGHT", timeout=60):
                    ...
                    logger.info(f"[Idempotency] REPLAY cached response: user={user_id} key={idem_key}")
                    return JsonResponse(cached["data"], status=cached["status"])
            except Exception as e:
                logger.error(f"[Idempotency] Pre-exec cache error: {e}")
                return view_func(self, request, *args, **kwargs) # Fail open

            # 5. EXECUTE VIEW
            response = view_func(self, request, *args, **kwargs)

            try:
                # 6. CACHE RESULT (2xx only)
                if 200 <= response.status_code < 300:
                    # Capture data for replay
                    ...
                    cache.set(cache_key, {"status": response.status_code, "data": response_data}, timeout=ttl)
                else:
                    # Errors shouldn't be idempotent - allow client to fix and retry.
                    cache.delete(cache_key)
            except Exception as e:
                logger.error(f"[Idempotency] Post-exec cache error: {e}")
