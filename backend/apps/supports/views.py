...
class CreateSupportTicketView(APIView):
    ...
    def post(self, request, ride_id):
        ...
        try:
            from apps.supports.services import open_support_ticket # Import here to avoid circular
...
class TriggerSOSView(APIView):
    ...
    def post(self, request, ride_id):
        ...
        if ride.rider != request.user and (not ride.driver or ride.driver.user != request.user):
            return Response({"error": "No permission to trigger SOS for this ride"}, status=403)
        ...
        # 1. To Admin
        async_to_sync(channel_layer.group_send)("admin_live_map", {
...
class ResolveEmergencyView(APIView):
    ...
    def post(self, request, emergency_id):
        ...
        from asgiref.sync import async_to_sync
        channel_layer = get_channel_layer()
        async_to_sync(channel_layer.group_send)(f"ride_{emergency.ride_id}", {
            "type": "sos_resolved",
            "data": {
                "emergency_id": emergency.id,
