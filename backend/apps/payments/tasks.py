...
def trigger_scheduled_payouts():
    ...
    active_drivers = Driver.objects.exclude(status=Driver.Status.BLOCKED).only(
        "id", "user"
    )
    ...
    if not CeleryQueueGuard.can_enqueue():
        logger.warning(
            "[Backpressure] Shedding scheduled payout tasks due to queue saturation"
        )
    ...
    try:
        with transaction.atomic():
            ...
            try:
                ...
                return f"Payout Failed: {e!s}"

    except Exception as e:
        # ── RETRY STRATEGY WITH JITTER ──
        # Use getattr/hasattr to safely check for Celery task context if called directly
        retries = getattr(getattr(self, "request", object()), "retries", 0)
        countdown = RetryStrategy.get_countdown("CRITICAL", retries)
        
        if countdown >= 0 and hasattr(self, "retry"):
...
def reconcile_processing_payouts():
    ...
    from apps.payments.models import Payout
    from apps.payments.services.payout import mark_payout_failed, mark_payout_success
    ...
    for payout in processing_payouts:
        try:
            ...
            if pg_status == "processed":
                ...
                payout.failure_reason = status_data.get(
                    "failure_reason", "Reconciled Failure"
                )
            ...
            send_critical_alert(
                ...
                message=f"Failed to reconcile payout {payout.id}: {e!s}",
                level="ERROR",
            )
    ...
    try:
        ...
        for payment in pending:
            try:
                ...
                captured_pg = next(
                    (i for i in items if i.get("status") == "captured"), None
                )
        ...
        return f"Reconciliation Failed: {e!s}"
...
def audit_platform_ledger():
    ...
    try:
        ...
        ).aggregate(total=Sum(F("amount") - F("refunded_amount")))["total"] or Decimal(
            "0.00"
        )
        ...
        driver_debits = LedgerEntry.objects.filter(
            ...
            reason__in=[
                ...
                LedgerEntry.Reason.CORRECTION,
                LedgerEntry.Reason.PENALTY,
            ],
        ).exclude(
        ...
        platform_credits = LedgerEntry.objects.filter(
            ...
            reason__in=[
                LedgerEntry.Reason.PLATFORM_COMMISSION,
                LedgerEntry.Reason.WITHDRAWAL_FEE,
            ],
        ).aggregate(total=Sum("amount"))["total"] or Decimal("0.00")
