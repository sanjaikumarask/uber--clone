...
class SimulatedPaymentView(APIView):
    ...
    def post(self, request, ride_id):
        logger.info(f"SIMULATE_PAYMENT: User {request.user.id} requesting for Ride {ride_id}")
        ...
        with transaction.atomic():
            ...
            if ride.rider != request.user and not request.user.is_admin:
                logger.warning(f"SIMULATE_PAYMENT: Permission Denied. Ride rider is {ride.rider_id}, but caller is {request.user.id}")
            ...
            if ride.status != Ride.Status.COMPLETED:
                return Response({"error": f"Ride is in {ride.status} state, cannot process payment"}, status=status.HTTP_400_BAD_REQUEST)
...
class CreatePaymentOrderView(APIView):
    ...
    def post(self, request, ride_id):
        ...
        if ride.rider != request.user and not request.user.is_admin:
            logger.warning(f"CREATE_PAYMENT: Permission Denied. Ride rider is {ride.rider_id}, but caller is {request.user.id}")
            return Response({"error": "Access denied. You are not the registered rider for this trip."}, status=status.HTTP_403_FORBIDDEN)

        if ride.status != Ride.Status.COMPLETED:
            return Response({"error": f"Ride is in '{ride.status}' state. Payment can only be initiated for completed rides."}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def post(self, request):
        ...
        with transaction.atomic():
            ...
            if payment.user != request.user and not request.user.is_admin:
                logger.warning(f"VERIFY_PAYMENT: Permission Denied. Payment user is {payment.user_id}, but caller is {request.user.id}")
                return Response({"error": "Unauthorized signature verification"}, status=status.HTTP_403_FORBIDDEN)
