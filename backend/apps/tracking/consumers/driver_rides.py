...
class DriverRidesConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        logger.info(
            "DriverRidesConsumer connected: driver=%s user=%s", driver.id, user.id
        )

        pending_offer = await self._get_pending_offer(driver)
        if pending_offer:
            logger.info(
                "Found pending offer for driver %s: ride %s",
                driver.id,
                pending_offer.id,
            )
            ...
            stats, _ = await database_sync_to_async(DriverStats.objects.get_or_create)(
                driver=driver
            )
                ...
                text_data=json.dumps(
                    {
                        ...
                        "data": {
                            ...
                            "rejections_until_auto": max(
                                0, 3 - stats.rejection_count_today
                            ),
                            "rider": {
                                "name": pending_offer.rider.get_full_name()
                                or pending_offer.rider.username,
    ...
    async def ride_offer(self, event):
        ...
        """
        logger.info(
            f"Forwarding ride_offer to Driver {self.driver.id}: Ride {event.get('data', {}).get('ride_id')}"
        )
        await self.send(
            text_data=json.dumps(
                {
    ...
    async def ride_assigned(self, event):
        ...
        """
        logger.info(
            f"Forwarding ride_assigned to Driver {self.driver.id}: Ride {event.get('data', {}).get('ride_id')}"
        )
        await self.send(
            text_data=json.dumps(
                {
    ...
    async def notify(self, event):
        ...
        """
        logger.info(
            f"Forwarding notification to Driver {self.driver.id}: {event.get('event')}"
        )
        await self.send(
            text_data=json.dumps(
                {
    ...
    def _get_pending_offer(self, driver):
        ...
        return (
            Ride.objects.filter(driver=driver, status=Ride.Status.OFFERED)
