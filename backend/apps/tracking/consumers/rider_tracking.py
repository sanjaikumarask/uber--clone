...
from apps.drivers.redis_rider import get_rider_last_point, update_rider_location
...
class RiderTrackingConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        if not self.is_admin and not await self._validate_ride(user):
            await self.close()
            return
        ...
        await self.channel_layer.group_send(
            self.user_group, {"type": "consumer_evicted"}
        )
        ...
        if ride:
            ...
            if ride.driver:
                payload["ride"]["driver"] = {
                    "id": ride.driver.id,
                    "lat": (
                        float(ride.driver.last_lat) if ride.driver.last_lat else None
                    ),
                    "lng": (
                        float(ride.driver.last_lng) if ride.driver.last_lng else None
                    ),
            ...
            if rider_loc:
                payload["ride"]["rider_loc"] = {
                    "lat": rider_loc[0],
                    "lng": rider_loc[1],
                }

            await self.send(
                text_data=json.dumps({"type": "WS_CONNECTED", "payload": payload})
            )
    ...
    async def receive(self, text_data):
        ...
        try:
            ...
            if msg_type == "SEND_CHAT":
                ...
                if lat and lng and not self.is_admin:
            ...
            text_data=json.dumps(
                {
                    ...
                    "payload": {
                        ...
                        "driver": {
                            ...
                            "lat": (
                                float(ride.driver.last_lat)
                                if (ride.driver and ride.driver.last_lat)
                                else None
                            ),
                            "lng": (
                                float(ride.driver.last_lng)
                                if (ride.driver and ride.driver.last_lng)
                                else None
                            ),
    ...
    def _validate_ride(self, user):
        ...
        return (
            ...
            .exclude(status__in=[Ride.Status.CANCELLED])
            .exists()
        )
