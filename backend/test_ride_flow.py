...
def run_test():
    ...
    try:
        ...
        print(
            f"✅ Users Found: Rider={rider.first_name}, Driver={driver.user.first_name}"
        )
    ...
    # Check Wallet
    rider_debit = (
        LedgerEntry.objects.filter(user=rider, entry_type="DEBIT")
        .order_by("-created_at")
        .first()
    )
    driver_credit = (
        LedgerEntry.objects.filter(user=driver.user, entry_type="CREDIT")
        .order_by("-created_at")
        .first()
    )
    ...
    print("\n💰 Wallet Entries:")
    print(
        f"   Rider Debit:  {rider_debit.amount if rider_debit else 'None'} ({rider_debit.reason if rider_debit else '-'})"
    )
    print(
        f"   Driver Credit: {driver_credit.amount if driver_credit else 'None'} ({driver_credit.reason if driver_credit else '-'})"
    )
