...
def fetch_directions_path(origin_lat, origin_lng, dest_lat, dest_lng):
    ...
    api_key = getattr(settings, "GOOGLE_MAPS_API_KEY", "") or os.environ.get(
        "GOOGLE_MAPS_API_KEY", ""
    )
    if not api_key:
    ...
    try:
        ...
        if data.get("status") != "OK":
            print(
                f"   [directions] API status: {data.get('status')} — using straight-line fallback."
            )
            return [(origin_lat, origin_lng), (dest_lat, dest_lng)]
...
def broadcast_location(
    driver, ride, lat, lng, channel_layer, speed=40, prev_lat=None, prev_lng=None
):
...
def broadcast_status(ride, channel_layer, extra=None):
    ...
    if ride.driver:
        ...
        admin_data["driver_name"] = (
            ride.driver.user.get_full_name() or ride.driver.user.username
        )
...
def follow_path(
    ...
    for i in range(total_points - 1):
        ...
        for s in range(steps_per_segment):
            ...
            bar = "#" * (pct // 5) + "." * (20 - pct // 5)
            print(
                f"\r   {label}: [{bar}] {pct:3d}%  ({lat:.5f}, {lng:.5f})  ",
                end="",
                flush=True,
            )
    ...
    print(
        f"\r   {label}: [####################] 100%  ({last_lat:.5f}, {last_lng:.5f})  "
    )
...
def simulate_trip(ride_id):
    ...
    ride = Ride.objects.select_related("driver", "driver__user", "rider").get(
        id=ride_id
    )
    driver = ride.driver
    ...
    if not main_path:
        ...
        main_path = fetch_directions_path(
            pickup_lat, pickup_lng, float(ride.drop_lat), float(ride.drop_lng)
        )
