...
def test_register_driver_cancellation_penalty(mock_atomic, mock_Stats_cls, mock_metrics, mock_suspend):
    driver = MagicMock()
...
@patch("apps.drivers.services.trust.DriverStats")
@patch("apps.drivers.services.trust.transaction.atomic")
def test_register_no_show_penalty(mock_atomic, mock_Stats_cls, mock_metrics, mock_suspend):
    driver = MagicMock()
    mock_stats = MagicMock()
...
def test_apply_progressive_suspension_logic(mock_timezone, mock_stats_save, mock_driver_save, mock_notify):
    from datetime import datetime, timezone as tz
    now = datetime(2025, 1, 1, 12, 0, 0, tzinfo=tz.utc)
