...
def test_update_location(mock_time, mock_redis, mock_validate):
    ...
    mock_redis.execute_command.assert_any_call("GEOADD", "drivers:geo", 20.0, 10.0, "1")

    # Verify Heartbeat
    mock_redis.setex.assert_called_with("driver:1:last_seen", 60, 1000)
...
def test_remove_driver(mock_redis):
    ...
    # Verify Geo Remove
    mock_redis.execute_command.assert_any_call("ZREM", "drivers:geo", "1")
