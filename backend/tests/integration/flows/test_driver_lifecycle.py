...
def test_driver_lifecycle_earnings(client):
    ...
    User.objects.create_user(
        pk=1, username="plaform_admin", password="p", role="admin", phone="+0000000000"
    )

    Driver.objects.get_or_create(
        user=driver_user, defaults={"status": Driver.Status.OFFLINE}
    )
    ...
    rider_user = User.objects.create_user(
        username="rider_life", password="password", role="rider", phone="+0987654321"
    )
    ...
    platform_entry = LedgerEntry.objects.filter(
        reference__startswith="commission:", ride_id=ride.id
    ).first()
