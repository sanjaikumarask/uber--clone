...
def test_payment_lifecycle_refund(mock_rp, client):
    ...
    driver_user = User.objects.create_user(
        username="driver_pay", password="p", role="driver", phone="+1112223333"
    )
    from apps.drivers.models import Driver

    Driver.objects.get_or_create(user=driver_user, defaults={"status": "OFFLINE"})
    driver = driver_user.driver

    rider_user = User.objects.create_user(
        username="rider_pay", password="p", role="rider", phone="+4445556666"
    )
    ...
    admin_user = User.objects.create_user(
        pk=1, username="admin_pay", password="p", role="admin", phone="+0000000000"
    )
    ...
    assert LedgerEntry.objects.filter(
        user=driver_user, entry_type="CREDIT"
    ).first().amount == Decimal("160.00")
    ...
    rider_refund = LedgerEntry.objects.filter(
        user=rider_user, ride_id=ride.id, entry_type="CREDIT", reason="bad_experience"
    ).first()
