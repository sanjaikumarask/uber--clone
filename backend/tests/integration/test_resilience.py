...
from apps.common.circuit_breaker import CircuitOpenException
...
class TestSystemResilience:
    ...
    def test_razorpay_circuit_breaker_trip(self):
        ...
        with patch("apps.payments.services.payout_gateway.client.post") as mock_post:
            ...
            # It should raise CircuitOpenException
    ...
    def test_payout_rollback_on_failure(self, driver_user):
        ...
        with patch("apps.payments.tasks.create_driver_payout") as mock_gateway:
            ...
            payout = Payout.objects.get(driver=driver_user, reference=f"payout:scheduled:{driver_user.id}:{timezone.now().date().isoformat()}")
            ...
            holds = LedgerEntry.objects.filter(user=driver_user, entry_type=LedgerEntry.Type.HOLD)
            releases = LedgerEntry.objects.filter(user=driver_user, entry_type=LedgerEntry.Type.RELEASE)
