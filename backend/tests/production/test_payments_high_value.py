...
class TestPaymentsHighValue:
    ...
    def test_settle_driver_payout_immutable_safety(self):
        ...
        User.objects.get_or_create(
            id=settings.PLATFORM_USER_ID, defaults={"username": "sys_ledger"}
        )
        ...
        credits = LedgerEntry.objects.filter(
            ride_id=self.ride.id, entry_type=LedgerEntry.Type.CREDIT
        ).count()
    ...
    @patch("apps.payments.views.razorpay_client")
    def test_reconcile_pending_payments_fixes_dead_battery_scenario(
        self, mock_razorpay
    ):
        """
        ...
        assert LedgerEntry.objects.filter(
            reference="earning:" + str(self.ride.id)
        ).exists()
    ...
    def test_trigger_scheduled_payouts_sheds_load_on_saturation(
        self, mock_enqueue, mock_balance
    ):
        """
