...
from apps.common.idempotency import IdempotencyMiddleware, idempotent_task, idempotent_webhook
...
class TestCommonIdempotencyResilience:
    ...
    def test_middleware_fail_open_on_redis_timeout(self):
        ...
        get_response = MagicMock(return_value=HttpResponse("Real Execution", status=200))
        
        with patch("django.core.cache.cache.add", side_effect=Exception("Redis Timeout")):
            middleware = IdempotencyMiddleware(get_response)
