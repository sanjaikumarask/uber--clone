...
from apps.payments.services.payout import request_driver_payout, mark_payout_success
...
class TestSeniorResilienceSuite:
    ...
    def setup_method(self):
        ...
        self.rider = User.objects.create_user(username=f"rider_{unique_id}", role="rider")
        self.driver_user = User.objects.create_user(username=f"driver_{unique_id}", role="driver")
        self.driver = self.driver_user.driver
        self.driver.status = Driver.Status.ONLINE
        self.driver.save()
        
        # Ensure Platform User exists for commissions
        User.objects.get_or_create(id=settings.PLATFORM_USER_ID, defaults={"username": f"plat_{unique_id}"})
    ...
    def test_reconcile_authorized_but_not_captured(self):
        ...
        payment = Payment.objects.create(
            user=self.rider, ride_id=1, amount=100.0, 
            status=Payment.Status.AUTHORIZED,
            gateway_order_id="recon_ord_1"
        )
        # Shift back to trigger threshold
        Payment.objects.filter(id=payment.id).update(created_at=timezone.now() - timezone.timedelta(minutes=30))
        
        with patch("apps.payments.views.razorpay_client") as mock_rz:
            mock_rz.order.payments.return_value = {"items": [{"id": "pay_xyz", "status": "captured"}]}
            
            # Setup ride for settlement
            Ride.objects.create(id=1, rider=self.rider, driver=self.driver, status=Ride.Status.COMPLETED, final_fare=100.0, pickup_lat=0, pickup_lng=0, drop_lat=0, drop_lng=0)
