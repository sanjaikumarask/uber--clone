...
class TestRidesApiHighValue:
    ...
    def test_estimate_fare_invalid_states(self):
        ...
        request = factory.post("/api/v1/rides/estimate/", {
            "pickup_lat": "invalid", "pickup_lng": 77.5946,
    ...
    def test_driver_arrived_but_not_assigned(self):
        ...
        ride = Ride.objects.create(
            rider=self.rider, driver=other_user.driver, status=Ride.Status.ASSIGNED,
        ...
        force_authenticate(request, user=self.driver_user) # CURRENT driver is NOT assigned
        
        view = DriverArrivedView.as_view()
        response = view(request, ride_id=ride.id)
        assert response.status_code == status.HTTP_404_NOT_FOUND # get_object_or_404(driver__user=request.user)
    ...
    def test_verify_otp_invalid_driver_permissions(self):
        ...
        factory = APIRequestFactory()
        request = factory.post(f"/api/v1/rides/{ride.id}/verify-otp/", {"otp": "1234"}, format="json")
