...
from apps.common.circuit_breaker import circuit_breaker, CircuitOpenException
from apps.common.idempotency import IdempotencyMiddleware, idempotent_webhook, idempotent_task
...
class TestReliabilityEngineering:
    ...
    def test_circuit_breaker_trip_and_bypass(self):
        ...
        @circuit_breaker(service_name=service_name, failure_threshold=failure_threshold, recovery_timeout=10)
        ...
        # 3. Fourth call - Should Bypassed (CircuitOpenException) immediately
        # No more 'ValueError' because we don't even call the func.
        with pytest.raises(CircuitOpenException, match="Circuit Breaker OPEN"):
            failing_api_call()
    ...
    def test_retry_exponential_backoff_jitter(self):
        ...
        times = []
        for _ in range(100):
            times.append(RetryStrategy.get_countdown("CRITICAL", attempt=3)) # 2 * 2^3 = 16
    ...
    def test_database_locking_contention_timeout(self, mock_select):
        ...
        driver_id = 1
        with patch("apps.drivers.models.Driver.objects.select_for_update") as mock_driver_select:
             ...
             with patch("apps.payments.tasks.process_driver_payout.retry") as mock_retry:
                 with patch("apps.common.backpressure.RetryStrategy.get_countdown", return_value=5):
