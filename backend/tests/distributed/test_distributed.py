...
class TestDistributedEdgeCases:
    ...
    def setup_method(self):
        ...
        self.user = User.objects.create_user(username=f"dist_user_{unique_id}", role="driver", phone=f"+999{unique_id}")
    ...
    def test_dispatch_race_condition_stale_read(self):
        unique_id = uuid.uuid4().hex[:8]
        rider = User.objects.create_user(username=f"rider_stale_{unique_id}", role="rider")
    ...
    def test_idempotent_task_crash_cleanup(self, mock_cache):
        ...
        with patch("apps.common.idempotency.hashlib.md5") as mock_md5:
            mock_md5.return_value.hexdigest.return_value = fingerprint
            dummy_task()
