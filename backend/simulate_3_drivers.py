...
def simulate_3_drivers():
    ...
    for data in driver_data:
        ...
        d, _ = Driver.objects.get_or_create(user=u, defaults={"id": data["id"], "status": "ONLINE"})
        d.status = "ONLINE"
        d.save()
        drivers.append(d)
    ...
    try:
        while True:
            ...
            print(f"📡 Updates @ {timestamp}: D1({positions[0]['lat']:.4f}), D2({positions[1]['lat']:.4f}), D3({positions[2]['lat']:.4f})", end="\r")
